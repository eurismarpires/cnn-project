...
muskAll = np.genfromtxt('/home/patcha/Datasets/Musk/musk.csv', delimiter=',')


#dnaTrain = np.concatenate((muskTrain,muskTest))

acc = list()
tim = list()

for i in range(30):
    musk = data (dataset=muskAll, percTrain=0.7, percVal=0.0, percTest=0.3, normType='mean', posOut='first')
    print musk
    ...
    init = time.time()    
    elmNet = KELM (inTrain=musk.trainIn, outTrain=musk.trainOut, kernelType ='pol')
    res, a = elmNet.train_and_test(musk.testIn, musk.testOut, aval=True)    
    end = time.time()    
    acc.append(a)
    tim.append(end-init)  
